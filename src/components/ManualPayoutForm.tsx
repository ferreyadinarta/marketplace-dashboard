...
export function ManualPayoutForm({
  ...
  const trigger = (
    ...
    >
      <Plus size={13} /> {t("Catat pembayaran", "Record payment")}
    </button>
  );
            ...
            >
              ...
              <div className="space-y-3 px-5 pb-5 pt-4 sm:px-6 sm:pt-5">
                <div>
                  <h3 className="text-[15px] font-semibold text-slate-900">
                    {t("Catat pembayaran diterima", "Record payment received")}
                  </h3>
                  <p className="mt-1 text-sm text-slate-500">
                    {t("Uang yang benar-benar masuk dari ", "Money that actually came in from ")}
                    <strong className="text-slate-700">{storeName}</strong>.{" "}
                    {t("Sisanya dianggap belum dibayar.", "The rest is treated as not yet paid.")}
                  </p>
                </div>

                <label className="block">
                  <span className="mb-1 block text-[11px] font-medium text-slate-500">{t("Tanggal terima", "Date received")}</span>
                  <DatePicker name="tanggal" defaultValue={today} />
                </label>

                <label className="block">
                  <span className="mb-1 block text-[11px] font-medium text-slate-500">{t("Jumlah diterima", "Amount received")}</span>
                  <CurrencyInput name="amount" value={amount} onValueChange={setAmount} placeholder="0" />
                </label>

                <label className="block">
                  <span className="mb-1 block text-[11px] font-medium text-slate-500">{t("Referensi (opsional)", "Reference (optional)")}</span>
                  <input
                    name="reference"
                    placeholder={t("ex: transfer BCA 12/8", "e.g. BCA transfer 12/8")}
              ...
              <div className="flex gap-2.5 border-t border-slate-100 bg-slate-50/70 px-5 py-3.5 sm:justify-end sm:px-6">
                ...
                <SubmitButton
                  ...
                  notify={t(`Pembayaran ${storeName} tercatat`, `Payment for ${storeName} recorded`)}
                  disabled={amount <= 0}
